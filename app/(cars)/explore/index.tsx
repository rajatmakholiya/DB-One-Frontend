...
import { LinearGradient } from "expo-linear-gradient"; // Make sure LinearGradient is imported
...
export default function SelectCarLocationScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();

  const selectLocation = (locationName: string) => {
    router.push({
      pathname: "/(cars)/explore/browse", // Navigate to the next step
          ...
          <>
            <View style={styles.headerContent}>
              ...
              <Text style={styles.subtitle}>
                Select a location to see available cars
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  cardImage: {
    width: "100%",
    height: width * 0.5 - 32, // Square card
    borderRadius: 18,
